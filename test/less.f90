program test_less
    ...
    !---
    if (is_less_than(weights_of_digits(3), weights_of_digits(4))) then
        print *, "PASSED: 8<16 is .true."
    else
        print *, "FAILED: expect that 8<16 is .true., but got .false."
    end if

    if (is_less_than(weights_of_digits(4), weights_of_digits(3)) .eqv. .false.) then
        print *, "PASSED: 16<8 is .false."
    else
        print *, "FAILED: expect that 16<8 is .false., but got .true."
    end if

    if (is_less_than(weights_of_digits(3), weights_of_digits(3)) .eqv. .false.) then
        print *, "PASSED: 8<8 is .false."
    else
        print *, "FAILED: expect that 8<8 is .false., but got .true."
    end if

    if (is_less_than(-weights_of_digits(4), -weights_of_digits(3)) .eqv. .true.) then
        print *, "PASSED: -16<-8 is .true."
    else
        print *, "FAILED: expect that -16<-8 is .true., but got .false."
    end if

    if (is_less_than(-weights_of_digits(3), -weights_of_digits(4)) .eqv. .false.) then
    ...
    !---
    if (is_less_than_or_equal_to(weights_of_digits(3), weights_of_digits(4))) then
        print *, "PASSED: 8<=16 is .true."
    else
        print *, "FAILED: expect that 8<=16 is .true., but got .false."
    end if

    if (is_less_than_or_equal_to(weights_of_digits(4), weights_of_digits(3)) .eqv. .false.) then
        print *, "PASSED: 16<=8 is .false."
    else
        print *, "FAILED: expect that 16<=8 is .false., but got .true."
    end if

    if (is_less_than_or_equal_to(weights_of_digits(3), weights_of_digits(3)) .eqv. .true.) then
        print *, "PASSED: 8<=8 is .true."
    else
        print *, "FAILED: expect that 8<=8 is .true., but got .false."
    end if

    if (is_less_than_or_equal_to(-weights_of_digits(4), -weights_of_digits(3)) .eqv. .true.) then
        print *, "PASSED: -16<=-8 is .true."
    else
        print *, "FAILED: expect that -16<=-8 is .true., but got .false."
    end if

    if (is_less_than_or_equal_to(-weights_of_digits(3), -weights_of_digits(4)) .eqv. .false.) then
        print *, "PASSED: -8<=-16 is .false."
    else
        print *, "FAILED: expect that -8<=-16 is .false., but got .true."
    end if
